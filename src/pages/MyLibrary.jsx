    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {books.map((item) => (
            <div key={item.id} className="flex gap-4 p-4 bg-white rounded-lg border border-gray-200 hover:border-indigo-200 transition">
              ...
              <div className="flex-1 min-w-0">
                <Link to={`/book/${item.books.open_library_key}`}>
                  <h3 className="font-semibold text-gray-900 hover:text-indigo-600 transition truncate">
                    {item.books.title}
                  </h3>
                </Link>
                <p className="text-sm text-gray-500 truncate">{item.books.author || 'Unknown Author'}</p>

                <div className="flex items-center gap-3 mt-3">
                  <select
                    value={item.status}
                    onChange={(e) => handleStatusChange(item.book_id, e.target.value)}
                    className="text-sm px-3 py-1.5 rounded-lg border border-gray-300 bg-white focus:ring-2 focus:ring-indigo-500 outline-none"
                  >
